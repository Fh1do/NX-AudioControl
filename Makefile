...
APP_TITLE         =    NX-AudioControl
APP_AUTHOR        =    "Fh1do"
APP_ICON          =
APP_VERSION       =    1.0.0
APP_TITLEID       =

TARGET            =    NX-AudioControl
EXTENSION         =    ovl
OUT               =    out
BUILD             =    build
SOURCES           =    src
...
DEFINES           =    __SWITCH__ APP_TITLE=\"$(APP_TITLE)\" APP_VERSION=\"$(APP_VERSION)\" APP_AUTHOR=\"$(APP_AUTHOR)\"
...
$(DIST_TARGET): | all
	@rm -rf $(OUT)/NX-AudioControl-*-*.zip
	@mkdir -p $(OUT)/switch/.overlays
	@cp $(NX_TARGET) $(OUT)/switch/.overlays
	@mkdir -p $(OUT)/atmosphere/exefs_patches/mastervolume_patches
	@cp misc/patches/*.ips $(OUT)/atmosphere/exefs_patches/mastervolume_patches || :
